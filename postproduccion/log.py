...
def _print_log(status, msg):
    ...
    f.write("%c[%s] %s\n" % (
        status,
        datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S.%f"), msg)
    )
    f.close()
...
def preview_enqueue(v):
    _print_log(
        INFO, "Se encola la previsualización de '%s' para ser codificada" % v)
...
def _register_entry():
    lock.acquire()
    current = int(config.get_option('CURRENT_LOG_SIZE')
                  ) if config.get_option('CURRENT_LOG_SIZE') else 0
