    ...
    def handle(self, *args, **options):
        ...
        if len(args) == 1:
            ...
            qs = Video.objects.filter(
                informeproduccion__fecha_produccion__year=year
            )
        else:
        ...
        print(u"Producciones validadas: %s (%s)" %
              (str(qs.filter(status='LIS').count()), d))

        # Píldoras
        d = video.get_duration(qs.exclude(plantilla=None))
        print(u"Píldoras: %s (%s)" %
              (str(qs.exclude(plantilla=None).count()), d))

        # Producciones propias
        d = video.get_duration(qs.filter(plantilla=None))
        print(u"Producciones propias: %s (%s)" %
              (str(qs.filter(plantilla=None).count()), d))
